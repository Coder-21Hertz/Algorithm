...
TreeNode* Convert(TreeNode* pRootOfTree)
{
    ...
    while(p != nullptr || !s.empty()){
        ...
        if(!s.empty()){
            ...
            if(pre != nullptr){
                p->left = pre;    //当前节点的左指针指向pre节点
                pre->right = p;    //将pre的右节点指向当前节点
            }
            else    //pre=nullptr表示第一次出栈，即中序遍历的第一个数
                pRootOfTree = p;//让头结点指向第一个遍历的数

            pre = p;    //用来记录前一节点，比当前结点小的
            p=p->right;
        }
    }
    return pRootOfTree; //记住是返回这个
}